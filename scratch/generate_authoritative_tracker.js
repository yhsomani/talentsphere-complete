...
// We will load the requirements catalog and create the authoritative tracker dataset
// Every item is directly tied to the specification section and verified against the codebase.

// Let's create the script to build and validate the entire tracker.
console.log('Validating requirements inventory...');
...
const modules = [
  ...
  { id: 24, name: 'Cross-Cutting Security Architecture', prefix: 'SEC', count: 8, section: '§23 Security Requirements' },
  { id: 25, name: 'Cross-Cutting Non-Functional Requirements', prefix: 'NFR', count: 7, section: '§29 Non-Functional' },
  { id: 26, name: 'Cross-Cutting SEO Requirements', prefix: 'SEO', count: 18, section: '§38 SEO Requirements' },
  { id: 27, name: 'Cross-Cutting DevOps & Operations', prefix: 'OPS', count: 8, section: '§34 Deployment & Ops' }
];

let sum = 0;
modules.forEach(m => {
  sum += m.count;
  console.log(`Category ${m.id}: ${m.name.padEnd(45)} [${m.prefix}] -> ${m.count}`);
});

console.log(`\nTOTAL REQUIREMENTS TRACKED: ${sum}`);
