...
export class CandidateService {
  ...
  async getProfile(userId: string): Promise<{
    ...
    const portfolioItems: PortfolioItem[] = (portRes.data || []).map((p: Record<string, unknown>) => ({
      ...
      media_urls: p.image_url ? [String(p.image_url)] : [],
      skills_demonstrated: [],
      started_at: p.start_date ? String(p.start_date) : String(p.created_at || ''),
      completed_at: p.end_date ? String(p.end_date) : undefined,
      is_featured: false,
      visibility: 'public',
  ...
  async addPortfolioItem(
    ...
    const { data, error } = await this.supabase
      ...
      .insert({
        ...
        start_date: portfolioItem.started_at ? portfolioItem.started_at.split('T')[0] : null,
        end_date: portfolioItem.completed_at ? portfolioItem.completed_at.split('T')[0] : null,
      })
      .select()
      .single();
