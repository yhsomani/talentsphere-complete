...
test.describe('E2E: Unified Multi-Entity Search & Autocomplete Discovery (F-147, BR-265, BR-266, BR-267)', () => {
  ...
  test('performs multi-entity cross-domain search across jobs, companies, and projects', async ({ request }) => {
    // 1. Search for jobs
    ...
    expect(jobBody.results.some((r: any) => r.type === 'job' && r.title.includes('Lead Distributed Systems'))).toBe(true);
    ...
    expect(compBody.results.some((r: any) => r.type === 'company' && r.title.includes('CyberCorp Innovations'))).toBe(true);
    ...
    expect(projBody.results.some((r: any) => r.type === 'project' && r.title.includes('Distributed Consensus Engine'))).toBe(true);
  ...
  test('delivers sub-100ms autocomplete suggestions with prefix and typo tolerance (BR-265, BR-266)', async ({ request }) => {
    ...
    expect(body.suggestions.some((s: any) => s.text.toLowerCase().includes('distributed'))).toBe(true);
  });

  test('applies multi-faceted filtering on location and minScore (BR-267)', async ({ request }) => {
    // Search with location matching Zurich
    const matchedLocRes = await request.get(`${API_BASE}/search?query=Architect&type=jobs&location=Zurich`);
    ...
    const unmatchLocRes = await request.get(`${API_BASE}/search?query=Architect&type=jobs&location=Sydney`);
  ...
  test('enforces zero-leakage privacy on private portfolio projects', async ({ request }) => {
    ...
    expect(candBody.results.some((r: any) => r.title.includes('Secret Internal NextGen Kernel'))).toBe(false);
    ...
    expect(pubBody.results.some((r: any) => r.title.includes('Secret Internal NextGen Kernel'))).toBe(false);
    ...
    expect(ownerBody.results.some((r: any) => r.title.includes('Secret Internal NextGen Kernel'))).toBe(true);
  });
});
