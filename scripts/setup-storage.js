...
async function setupStorage() {
  ...
  await client.query("UPDATE storage.buckets SET public = true WHERE id IN ('avatars', 'portfolio');");
  ...
  const policies = [
    ...
    {
      name: 'Users can read own resumes',
      sql: `CREATE POLICY "Users can read own resumes" ON storage.objects FOR SELECT USING (bucket_id = 'resumes' AND auth.uid()::text = (storage.foldername(name))[1]);`
    },
    {
